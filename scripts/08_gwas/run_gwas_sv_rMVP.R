...
geno_file <- "ref_and_bam_bai/sv_matrix_num_forGWAS_min20.txt"
map_file_numeric <- "ref_and_bam_bai/sv_map_forGWAS_min20.numeric.txt"
map_file_full <- "ref_and_bam_bai/sv_map_forGWAS_min20.txt"
...
stopifnot(all(c("SNP", "Chrom", "Position") %in% names(map)))

map$ChromNum <- suppressWarnings(
  as.integer(sub("^[Cc]hr([0-9]+).*", "\\1", map$Chrom))
)

cat("Example chromosome names from map:\n")
print(head(map$Chrom, 10))
...
map_mvp <- data.frame(
  SNP      = map$SNP,
  Chrom    = map$ChromNum,
  Position = suppressWarnings(as.integer(map$Position)),
  stringsAsFactors = FALSE,
  check.names = FALSE
)
...
keep_non_na <- !is.na(map_mvp$SNP) & is.finite(map_mvp$Position)
if (any(!keep_non_na)) {
...
stopifnot(any(!is.na(map_mvp$Chrom)))
...
if (any(dup_mask)) {
  ...
  write.csv(id_map, "id_mapping_min20.csv", row.names = FALSE)
...
glm_out <- paste0("mvp_out_glm_", trait_name, "_min20")
mlm_out <- paste0("mvp_out_mlm_", trait_name, "_min20")
...
imap_file <- "id_mapping_min20.csv"

if (file.exists(sig_file) && file.exists(imap_file)) {
  sig  <- read.csv(sig_file, check.names = FALSE)
  imap <- read.csv(imap_file, check.names = FALSE)

  if (!("SNP" %in% names(sig))) {
    stop("Column 'SNP' not found in GWAS signal file.")
  }

  if (!all(c("SNP_new", "SNP_old", "Chrom", "Position") %in% names(imap))) {
    stop("Expected columns missing in id_mapping_min20.csv.")
  }

  ann <- sig %>%
    dplyr::rename(SNP_new = SNP, p = `Trait.MLM`) %>%
    dplyr::left_join(imap, by = "SNP_new")

  chrom_col <- dplyr::case_when(
    "Chrom"   %in% names(ann) ~ "Chrom",
    "Chrom.x" %in% names(ann) ~ "Chrom.x",
    "Chrom.y" %in% names(ann) ~ "Chrom.y",
    TRUE ~ NA_character_
  )

  pos_col <- dplyr::case_when(
    "Position"   %in% names(ann) ~ "Position",
    "Position.x" %in% names(ann) ~ "Position.x",
    "Position.y" %in% names(ann) ~ "Position.y",
    TRUE ~ NA_character_
  )

  if (is.na(chrom_col) || is.na(pos_col)) {
    cat("[WARNING] Chromosome/position columns could not be identified after joining results.\n")
  } else {
    ann <- ann %>%
      dplyr::mutate(
        CHROM_vcf = paste0("chr", .data[[chrom_col]], "__REFERENCE"),
        POS_vcf   = .data[[pos_col]]
      )

    out_hits_csv <- paste0("hits_", trait_name, "_min20_annot.csv")
    out_regions  <- paste0("hits_", trait_name, "_min20.regions.tsv")

    write.csv(ann, out_hits_csv, row.names = FALSE)

    regions <- ann %>%
      dplyr::transmute(CHROM_vcf, START = POS_vcf, END = POS_vcf)
    ...
    cat("Annotated hits written to:", out_hits_csv, "\n")
    cat("bcftools regions file written to:", out_regions, "\n")
  }
} else {
  cat("[WARNING] Signal or ID mapping file not found. Skipping annotated hit export.\n")
}
...
p_hist <- ggplot(phe, aes(x = Trait)) +
  ...
  labs(
    x = "BLUP phenotype",
    y = "Density",
    title = paste0("BLUP distribution: ", trait_name)
  ) +
  theme_bw()

ggsave(
  paste0("plots/BLUP_distribution_", trait_name, ".png"),
...
p_qq <- ggplot(phe, aes(sample = Trait)) +
  stat_qq() +
  stat_qq_line() +
  labs(title = paste0("BLUP QQ plot: ", trait_name)) +
  theme_bw()

ggsave(
  paste0("plots/BLUP_QQ_", trait_name, ".png"),
...
if (file.exists(mlm_all_file)) {
  ...
  png(
    paste0("plots/QQ_MLM_", trait_name, ".png"),
  ...
  plot(
    ...
    main = paste0("QQ plot MLM (lambda=", round(lambda_gc, 3), ") - ", trait_name)
  )
  abline(0, 1, lwd = 2)
  dev.off()
